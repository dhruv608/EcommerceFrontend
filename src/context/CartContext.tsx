...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  const fetchCart = async () => {
    ...
    try {
      ...
      const processedItems = cartData.map((item: any) => ({
        ...
        subtotal: item.subtotal || (item.productPrice || 0) * (item.quantity || 1),
  ...
  const updateQuantity = async (productId: number, quantity: number) => {
    ...
    try {
      setLoading(true);
      console.log("Updating cart using remove and re-add method:", { userId: user.userId, productId, quantity });
