...
import { Minus, Plus, Trash2, ShoppingBag, ArrowLeft, Truck, Shield, CreditCard, ShoppingCart } from "lucide-react";
import Link from "next/link";
import { useRouter } from "next/navigation";
import Image from "next/image";

export default function CartPage() {
  const { cartItems, cartCount, cartTotal, loading, updateQuantity, removeFromCart, clearCart } = useCart();
      ...
      <div className="bg-card sticky top-0 z-10">
        <div className="container mx-auto px-4 py-6">
          <div className="flex items-center justify-between">
            <Link href="/products" className="flex items-center gap-2 text-gray-600 text-sm hover:text-[#acac49]">
              ...
              <Card key={item.cartId} className="card-premium overflow-hidden card-hover">
                <CardContent className="p-4 sm:p-0">
                  <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 p-4 sm:p-6">
                    {/* Product Image */}
                    <Link href={`/products/${item.productId}`} className="w-full sm:w-32 h-32 sm:h-32 bg-muted rounded-lg flex-shrink-0 relative overflow-hidden block hover:opacity-90 transition-opacity max-h-[140px]">
                      {item.imageUrl ? (
                        <Image
                          src={item.imageUrl}
                    ...
                    <div className="flex-1 space-y-3 sm:space-y-4">
                      <div>
                        <Link href={`/products/${item.productId}`} className="text-base sm:text-lg font-semibold text-gray-900 mb-2 hover:underline block leading-tight">
                          {item.productName}
                        </Link>
                      ...
                      <div className="flex items-center justify-between mb-3 sm:mb-4">
                        <div>
                          ...
                          <p className="text-sm sm:text-base font-semibold text-gray-900">${(item.productPrice || 0).toFixed(2)}</p>
                        </div>
                        <Button
                          variant="ghost"
                          size="sm"
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center border border-border rounded-lg">
                          ...
                          <span className="w-12 sm:w-16 text-center font-bold text-sm sm:text-lg text-foreground">{item.quantity || 1}</span>
                          <Button
                            variant="ghost"
                            size="sm"
            ...
            <Card className="card-premium sticky top-24">
              <CardContent className="p-6">
                ...
                <div className="space-y-3 mt-8">
                  <Link href="/checkout">
                    <Button className="w-full bg-[#acac49] hover:bg-[#96963f] text-white rounded-lg px-6 py-3 font-medium shadow-sm" size="lg">
                  ...
                  <Link href="/products">
                    <Button variant="outline" className="w-full border border-gray-300 bg-white text-gray-700 rounded-lg px-6 py-3 hover:bg-gray-50">
                      Continue Shopping
                    </Button>
                  </Link>
