...
export default function Navbar() {
  ...
  const handleCategoryClick = (categoryId: string) => {
    ...
    logger.log('Category clicked:', categoryId)
    logger.log(
      'Category object:',
  ...
  return (
    <>
      <nav
        className={`nav-premium ${scrolled ? 'shadow-premium-lg' : ''} transition-all duration-300`}
      >
        ...
        {/* Mobile Menu */}
        <div
          className={`lg:hidden transition-all duration-300 ${mobileMenuOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0 overflow-hidden'}`}
        >
